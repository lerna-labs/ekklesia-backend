...
Stop logging the MongoDB password in clear text when `backup/restore.js` runs a restore; the `mongorestore` command it builds is still logged for its target database, but the command string itself (including the password) is no longer printed.
