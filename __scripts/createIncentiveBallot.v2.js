...
const authoredAtDate = authoredAt ? parseIsoOrExit('--authoredAt', authoredAt) : new Date(now);
...
if (doPrepare) {
  ...
  if (!endpoint) {
    console.error('[rss-v2] --prepare requires --endpoint=<url> or HYDRA_DEFAULT_ENDPOINT in env.');
    await disconnectFromDatabase();
    process.exit(2);
  }
  try {
  ...
  console.log(
    `  node -e "import('./__scripts/scaffold/common/hydraPrepare.js').then(async m => { const {Ballot}=await import('./schema/Ballot.js'); const b=await Ballot.findById('${ballot._id}'); console.log(JSON.stringify(await m.buildPrepareBody(b), null, 2)); })"`,
  );
}
