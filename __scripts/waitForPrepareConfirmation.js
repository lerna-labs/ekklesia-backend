...
if (!apiUrl || !apiToken) {
  console.error("Koios not configured — set API_URL and API_TOKEN in .env.development / .env.local");
  process.exit(1);
}
...
if (!txHash) {
  ...
  if (!ballot.prepareTxHash) {
    console.error(
      `Ballot ${ballot.title} has no prepareTxHash — has /prepare been called?`
    );
    await disconnectFromDatabase();
    process.exit(1);
  }
