...
console.log(`[refresh] matched ${ballots.length} ballot(s)${touchAll ? " (--all)" : ` with title prefix "${titlesPrefix}"`}`);
if (ballots.length === 0) {
  await teardown();
  process.exit(0);
}
...
for await (const v of voteCursor) {
  ...
  await Vote.updateOne(
    { _id: v._id },
    { $set: { submittedAt: new Date(now - offset) } }
  );
  touched++;
}
