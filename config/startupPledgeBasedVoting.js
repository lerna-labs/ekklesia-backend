...
export async function startupBallot(ballotId) {
  ...
  for (const pool of poolTotals.poolData) {
    ...
    const livePledge =
      pool.live_pledge === null || pool.live_pledge === undefined
        ? BigInt(0)
        : typeof pool.live_pledge === 'string'
          ? BigInt(pool.live_pledge)
          : BigInt(pool.live_pledge || 0);
    const pledge =
      pool.pledge === null || pool.pledge === undefined
        ? BigInt(0)
        : typeof pool.pledge === 'string'
          ? BigInt(pool.pledge)
          : BigInt(pool.pledge || 0);

    if (livePledge < pledge) {
      console.log('Pool live pledge is smaller than pledge: ', pool.live_pledge, pool.pledge);
      poolTotals.poolData = poolTotals.poolData.filter(
        (p) => p.pool_id_bech32 !== pool.pool_id_bech32,
      );
    }
  }
  ...
  for (const pool of poolTotals.poolData) {
    await UserCache.findOneAndUpdate(
      ...
      { votingPower: pool.live_pledge, validated: true },
      { upsert: true },
    );
  }
