...
router.get('/ballots', isAuthenticated, async (req, res) => {
  ...
  const ballotsToValidate = liveBallots.filter((ballot) => !votedBallots.includes(String(ballot)));
  ...
  const ballots = await Ballot.find({
    _id: { $in: voterBallots },
  }).select('_id title description voterType status votePeriodStart voteWeighted votePeriodEnd');
 ...
 */
router.post('/:ballotId/checkout/', isAuthenticated, getBallot, async (req, res) => {
  const userId = req.userId;
  ...
  if (ballot.status !== 'live') {
    console.log('Checkout attempt (post) from ' + userId, 'CLOSED Ballot: ' + ballotId.toString());
  ...
  if (!checkUserCache || checkUserCache.validated === false) {
    console.log('Voter is not allowed to vote on this ballot', 'UserCache: ', checkUserCache);
    return res.status(400).json({
      status: 'error',
 ...
 * @returns {Object} 404 - Error if ballot not found (handled by getBallot middleware)
 */
router.put('/:ballotId/checkout', isAuthenticated, getBallot, async (req, res) => {
  ...
  // check if ballot is live
  if (ballot.status !== 'live') {
    console.log('Checkout attempt (put) from ' + userId, 'CLOSED Ballot: ' + ballotId.toString());
    return res.status(400).json({
      status: 'error',
      message: 'The Voting Period for this ballot has ended',
    });
  ...
  console.log('Votes submitted for ' + userId, 'Ballot: ' + ballotId.toString());
...
const checkoutMultisigPost = async (req, res) => {
  ...
  let validatedScriptAddress = await validateAddress(req.body.scriptAddress.trim(), 'drep');
  if (!validatedScriptAddress || validatedScriptAddress.error) {
    console.error('MS: Script address validation error', validatedScriptAddress);
    return res.status(400).json({
      status: 'error',
  ...
  if (!validatedScriptAddress.isScript) {
    console.error('MS: Given address is not a script address', validatedScriptAddress);
    return res.status(400).json({
      status: 'error',
...
router.post('/:ballotId/checkout/multisig', isAuthenticated, getBallot, checkoutMultisigPost);
 ...
 * @returns {Object} 404 - Error if ballot not found (handled by getBallot middleware)
 */
router.put('/:ballotId/checkout/multisig', isAuthenticated, getBallot, async (req, res) => {
  console.log('Submit transaction for ' + req.userId);
  const userId = req.userId;
  const ballot = req.ballot;
  const ballotId = ballot._id;
  ...
  if (!isParty || isParty.error) {
    console.error('isPartyToScript verification failed', signerAddress, userId, isParty);
  ...
  if (multisigComplete) {
    ...
    console.log('MultiSig: Votes submitted for ' + userId, 'Ballot: ' + ballotId.toString());
