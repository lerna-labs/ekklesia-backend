    ...
    if (failures.length > 0) {
        ...
        failures.forEach(f => {
            if (f.koiosValue !== undefined && f.blockfrostValue !== undefined) {
                ...
                console.error(`🔴 FAIL: ${f.ticker} (${f.poolId}) - ${label1}: ${f.koiosValue}, ${label2}: ${f.blockfrostValue}`);
            } else {
...
runTest("TEST2: Koios pool_status 'registered' should have empty Blockfrost retirement array", () => {
    const mismatches = [];
    for (const pool of pools) {
        const koiosStatus = pool.koios_pool_info?.pool_status;
        ...
        if (koiosStatus === "registered") {
            if (blockfrostRetirement && Array.isArray(blockfrostRetirement) && blockfrostRetirement.length > 0) {
                mismatches.push({
                    ticker: pool.ticker,
                    poolId: pool.pool_id_bech32,
                    koiosValue: koiosStatus,
...
runTest("TEST4: live_pledge comparison between Koios and Blockfrost", () => {
    ...
    for (const pool of pools) {
        ...
        const koiosValue = koiosLivePledge !== null && koiosLivePledge !== undefined
            ? String(koiosLivePledge)
            : null;
        const blockfrostValue = blockfrostLivePledge !== null && blockfrostLivePledge !== undefined
...
runTest("TEST5: pledge comparison between Koios (pledge) and Blockfrost (declared_pledge)", () => {
    ...
    for (const pool of pools) {
        ...
        // Convert both to strings for comparison (handle null as "null" or empty string)
        const koiosValue = koiosPledge !== null && koiosPledge !== undefined
            ? String(koiosPledge)
            : null;
        const blockfrostValue = blockfrostPledge !== null && blockfrostPledge !== undefined
            ? String(blockfrostPledge)
            : null;
...
runTest("TEST7: live_stake comparison between Koios and Blockfrost", () => {
    ...
    for (const pool of pools) {
        ...
        // Convert both to strings for comparison (handle null as "null" or empty string)
        const koiosValue = koiosLiveStake !== null && koiosLiveStake !== undefined
            ? String(koiosLiveStake)
            : null;
        const blockfrostValue = blockfrostLiveStake !== null && blockfrostLiveStake !== undefined
