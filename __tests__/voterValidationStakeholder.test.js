...
runDescribe('voterValidationStakeholder (mongo)', () => {
  beforeAll(async () => {
    ...
    try {
      ...
      console.warn(`[voterValidationStakeholder] skipping: Mongo unreachable (${err.message}).`);
    }
  }, 10_000);
  ...
  async function createBallot(title, requirements = null) {
    return Ballot.create({
      ...
      status: 'live',
      source: 'hydra',
      voterGroups: [{ group: 'stake', powerSource: 'StakeBased', requirements }],
    });
  }
  ...
  maybe('enforces tokenHoldings threshold', async () => {
    const b = await createBallot(`stake-test-${runId}-e`, {
      tokenHoldings: [{ policyId: 'aa'.repeat(28), assetName: 'deadbeef', minQuantity: '100' }],
    });
    mockAccountInfo.mockResolvedValue({
      status: 'registered',
