...
describe('buildFacetQuery — filters', () => {
  ...
  test('single-value enum CSV → $in', () => {
    ...
    expect(r.filter['externalProposal.snapshot.facets.region'].$in).toEqual(['LATAM', 'APAC']);
  });
  ...
  test('number filter rejects non-number', () => {
    expect(() => buildFacetQuery(ballot, { filter: { totalCost: 'abc' } })).toThrow(
      /finite number/,
    );
  });
  ...
  test('boolean rejects CSV of values', () => {
    expect(() => buildFacetQuery(ballot, { filter: { featured: 'true,false' } })).toThrow(
      /multiple values/,
    );
  });

  test('unknown filter key rejected', () => {
    expect(() => buildFacetQuery(ballot, { filter: { bogus: 'x' } })).toThrow(/unknown filter key/);
  });

  test('non-filterable facet rejected', () => {
    expect(() => buildFacetQuery(ballot, { filter: { hidden: 'x' } })).toThrow(/not filterable/);
  });

  test('filter value outside enum options rejected', () => {
    expect(() => buildFacetQuery(ballot, { filter: { category: 'education,rocketry' } })).toThrow(
      /unknown options/,
    );
  });
