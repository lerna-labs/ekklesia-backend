...
function upstreamError(status, body = {}) {
  return {
    ok: false,
    status,
    json: async () => ({ status: "ERROR", code: body.code || null, message: body.message || "bad", data: body.data || null }),
...
describe("hydraClient transport", () => {
  ...
  test("surfaces ERROR envelope as HydraClientError", async () => {
    global.fetch = jest.fn(async () => ({
      ok: true,
      status: 200,
      json: async () => ({ status: "ERROR", code: "HEAD_CLOSED", message: "Head is closed", data: null }),
      text: async () => "",
    }));
    const client = forEndpoint("http://hydra.example");
