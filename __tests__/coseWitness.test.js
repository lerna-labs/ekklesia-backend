...
const DREP01 = {
  ...
  expectedPublicKey:
    "9d3c60458a81854624924a59544ee278ec1c7fa1ea69d0c4a3f27fcce5274dd8",
...
describe("coseWitness.normalizeWitness", () => {
  ...
  test("throws on missing coseSign1Hex", () => {
    expect(() => normalizeWitness({ coseKeyHex: DREP01.coseKeyHex })).toThrow(
      CoseWitnessError
    );
  });
  ...
  test("throws on missing coseKeyHex", () => {
    expect(() =>
      normalizeWitness({ coseSign1Hex: DREP01.coseSign1Hex })
    ).toThrow(CoseWitnessError);
  });

  test("throws on garbage CBOR in coseKeyHex", () => {
    expect(() =>
      normalizeWitness({ coseSign1Hex: DREP01.coseSign1Hex, coseKeyHex: "zz" })
    ).toThrow(CoseWitnessError);
  });
});
...
describe("coseWitness.keyHashFromPublicKeyHex", () => {
  test("matches the drep01 fixture hash", () => {
    expect(keyHashFromPublicKeyHex(DREP01.expectedPublicKey)).toBe(
      DREP01.expectedKeyHash
    );
  });
});
...
describe("coseWitness.publicKeyFromCoseKey", () => {
  test("extracts the -2 field from a COSE key CBOR map", () => {
    expect(publicKeyFromCoseKey(DREP01.coseKeyHex)).toBe(
      DREP01.expectedPublicKey
    );
  });
});
