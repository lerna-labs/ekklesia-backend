...
export async function rollupBallot($results, $weight, $epoch_no) {
  ...
  for (const proposal of $results.proposals) {
    ...
    for (const vote of proposal.votes) {
      ...
      switch (proposal.voteType) {
        ...
        default:
          ...
          vote_value =
            submittedArr.length > 1 ? JSON.stringify(submittedArr) : submittedArr[0].toString();
          break;
      }

      const formatted_vote = [proposal_id, voter_key_id, vote_value];
      ...
      if (results.voter_weights[voter_key_id] === undefined) {
        if ($weight) {
          ...
          if (results.voter_weights[voter_key_id] === undefined) {
            ...
            // zero-weight voter
            results.voter_weights[voter_key_id] = Number(0).toString();
          }
        } else {
          // If no weights are supplied, all voters have 1 "weight"
          results.voter_weights[voter_key_id] = Number(1).toString();
        }
      }
    ...
    for (const [key, val] of Object.entries(result_proposal.stats.thresholds)) {
      result_proposal.stats.thresholds[key] =
        result_proposal.weighted_results[key] / result_proposal.stats.total;
    }

    if (odd_num_votes) {
      // Preventing a strange error w/ odd-numbered merkle trees
      prepared_votes.push(prepare_vote([null, null, null]));
    }
  ...
  for (const proposal of ballot.proposals) {
    ...
    proposal.votes = proposal.votes.map((vote) => {
      ...
      return [vote, proposalTree.getHexProof(hashFunction(prepared))];
    });
  }
