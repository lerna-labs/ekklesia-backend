...
describe("projectVoteEntries", () => {
  ...
  test("multi-choice → multiple labels, drops unknown ids", () => {
    expect(
      projectVoteEntries(["alice", "carol", "ghost"], multiChoiceProposal)
    ).toEqual(["Alice", "Carol"]);
  });
  ...
  test("likert with unknown option falls back to id as label", () => {
    expect(
      projectVoteEntries([{ option: "phantom", value: 3 }], likertProposal)
    ).toEqual([{ option: "phantom", optionLabel: "phantom", value: 3 }]);
  });
  ...
  test("ObjectId-like ids compared by toString", () => {
    ...
    expect(
      ...
      { option: idA, optionLabel: "Onboarding is clear", value: 5 },
    ]);
  });
});
