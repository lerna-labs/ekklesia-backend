...
describe('canonicalSpaPath', () => {
  ...
  test('results view appended when flagged', () => {
    expect(canonicalSpaPath({ ballot: b, proposal: p, resultsView: true })).toBe(
      `/ballots/${b._id}/proposals/${p._id}/results`,
    );
  });
});
...
describe('canonicalApiPath', () => {
  ...
  test('ballot family', () => {
    ...
    expect(canonicalApiPath('ballot-archive', id)).toBe(`/api/v1/ballots/${id}/archive`);
    expect(canonicalApiPath('ballot-certified', id)).toBe(`/api/v1/ballots/${id}/certified`);
  });
  ...
  test('proposal family', () => {
    expect(canonicalApiPath('proposal', id)).toBe(`/api/v1/proposals/${id}`);
    expect(canonicalApiPath('proposals-by-ballot', id)).toBe(`/api/v1/proposals/ballot/${id}`);
  });
