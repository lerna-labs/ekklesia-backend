...
const { spaCanonicalRedirect, _internals } = await import('../spaCanonicalRedirect.js');
...
describe('spaCanonicalRedirect', () => {
  ...
  test('preserves the query string on redirect', async () => {
    ...
    const res = mkRes();
    await spaCanonicalRedirect(req, res, () => {});
    expect(res.redirected).toBe(`/ballots/${canonical}?utm_source=email`);
  });
  ...
  test('301-redirects when only the proposal segment is external', async () => {
    ...
    await spaCanonicalRedirect(req, res, () => {});
    expect(res.statusCode).toBe(301);
    expect(res.redirected).toBe(`/ballots/${ballot}/proposals/${proposal}`);
  });
  ...
  test('propagates the /results suffix on the canonical URL', async () => {
    ...
    await spaCanonicalRedirect(req, res, () => {});
    expect(res.statusCode).toBe(301);
    expect(res.redirected).toBe(`/ballots/${ballot}/proposals/${proposal}/results`);
  });
