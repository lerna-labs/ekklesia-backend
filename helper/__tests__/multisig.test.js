...
import { isPartyToScript, validateScriptSignatures } from '../verifySignature.js';
...
describe('Native Script Tests', () => {
  ...
  describe('invalid arguments', () => {
    test('Missing payload', async () => {
      expect(await validateScriptSignatures()).toEqual({
        error: 'Payload is missing',
      });
    });

    test('Missing address', async () => {
      expect(await validateScriptSignatures(generic_payload)).toEqual({
        error: 'Signer address is not provided',
      });
    });

    test('Missing signatures', async () => {
      expect(await validateScriptSignatures(generic_payload, ms.one.cip105)).toEqual({
    ...
    test('Signature must be an array', async () => {
      expect(
        await validateScriptSignatures(generic_payload, ms.one.cip105, std_signatures.one),
      ).toEqual({
        error: 'Signatures must be an array',
      });
    });
  ...
  describe('native script scenario #1: Time-locked Single Signer', () => {
    test('Key cannot sign in to Script', async () => {
      expect(
        await isPartyToScript(generic_payload, ms.one.cip129, std_signatures.two, ms.one.script),
      ).toEqual({
    ...
    describe('CIP-105 Tests', () => {
      test('Address Validation', () => {
        expect(validateAddress(ms.one.cip105, 'drep')).toEqual({
          cip105: 'drep_script19tqfdwrqadq8l762392779wrwa97f33j7mfnzzf97gpx7n6h8n2',
          cip129: 'drep1yv4vp94cvr45qllmf2y4tmc4cdm5hexxxtmdxvgfyheqymcz7rw5m',
          isScript: true,
        });
      ...
      test('Can Sign in with Script', async () => {
        expect(
          await isPartyToScript(generic_payload, ms.one.cip105, std_signatures.one, ms.one.script),
        ).toEqual(true);
      });
      ...
      test('COSE Sign can sign in with Script', async () => {
        expect(
          await isPartyToScript(generic_payload, ms.one.cip105, cose_signatures.one, ms.one.script),
        ).toEqual(true);
      });
    });
    ...
    describe('CIP-129 Tests', () => {
      test('Address Validation', () => {
        expect(validateAddress(ms.one.cip129, 'drep')).toEqual({
          cip105: 'drep_script19tqfdwrqadq8l762392779wrwa97f33j7mfnzzf97gpx7n6h8n2',
          cip129: 'drep1yv4vp94cvr45qllmf2y4tmc4cdm5hexxxtmdxvgfyheqymcz7rw5m',
          isScript: true,
        });
      ...
      test('Can Sign In with Script', async () => {
        expect(
          await isPartyToScript(generic_payload, ms.one.cip129, std_signatures.one, ms.one.script),
        ).toEqual(true);
      });
      ...
      test('COSE Sign can sign in with Script', async () => {
        expect(
          await isPartyToScript(generic_payload, ms.one.cip129, cose_signatures.one, ms.one.script),
        ).toEqual(true);
      });
    });
  });
  ...
  describe('native script scenario #2: All of Two Signers', () => {
    ...
    describe('CIP-105', () => {
      test('Signer #1 Can Authenticate Script', async () => {
        expect(
          await isPartyToScript(generic_payload, ms.two.cip105, std_signatures.one, ms.two.script),
        ).toBe(true);
      });

      test('Signer #2 Can Authenticate Script', async () => {
        expect(
          await isPartyToScript(generic_payload, ms.two.cip105, std_signatures.two, ms.two.script),
        ).toBe(true);
      });
    ...
    describe('CIP-129', () => {
      test('Signer #1 Can Authenticate Script', async () => {
        expect(
          await isPartyToScript(generic_payload, ms.two.cip129, std_signatures.one, ms.two.script),
        ).toBe(true);
      });

      test('Signer #2 Can Authenticate Script', async () => {
        expect(
          await isPartyToScript(generic_payload, ms.two.cip129, std_signatures.two, ms.two.script),
        ).toBe(true);
      });
  ...
  describe('native script scenario #4: At Least One Signer', () => {
    test('Signer #1 Can Authenticate Script', async () => {
      expect(
        await isPartyToScript(generic_payload, ms.four.cip105, std_signatures.one, ms.four.script),
      ).toBe(true);
    });

    test('COSE Signer #2 Can Authenticate Script', async () => {
      expect(
        await isPartyToScript(generic_payload, ms.four.cip129, cose_signatures.two, ms.four.script),
      ).toBe(true);
    });
  ...
  describe('native script scenario #5: At Least Two Signers', () => {
    test('Signer #1 Can Authenticate Script', async () => {
      expect(
        await isPartyToScript(generic_payload, ms.five.cip105, std_signatures.one, ms.five.script),
      ).toBe(true);
    });

    test('COSE Signer #2 Can Authenticate Script', async () => {
      expect(
        await isPartyToScript(generic_payload, ms.five.cip129, cose_signatures.two, ms.five.script),
      ).toBe(true);
    });
  ...
  describe('native script scenario #6: At Least All Signers', () => {
    test('Signer #1 Can Authenticate Script', async () => {
      expect(
        await isPartyToScript(generic_payload, ms.six.cip105, std_signatures.one, ms.six.script),
      ).toBe(true);
    });

    test('COSE Signer #2 Can Authenticate Script', async () => {
      expect(
        await isPartyToScript(generic_payload, ms.six.cip129, cose_signatures.two, ms.six.script),
      ).toBe(true);
    });

    test('Signer #3 Can Authenticate Script', async () => {
      expect(
        await isPartyToScript(generic_payload, ms.six.cip105, std_signatures.three, ms.six.script),
      ).toBe(true);
    });
  ...
  describe('native script scenario #9: Mesh Single Key', () => {
    test('CLI-Key Signature Can Authenticate Script', async () => {
      expect(
        await isPartyToScript(generic_payload, ms.mesh.cip129, std_signatures.mesh, ms.mesh.script),
      ).toBe(true);
    });
